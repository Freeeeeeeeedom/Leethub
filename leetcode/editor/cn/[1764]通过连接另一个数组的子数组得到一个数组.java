...
// 
//
// Related Topics 贪心 数组 字符串匹配 👍 28 👎 0
...
class Solution {
    public boolean canChoose(int[][] groups, int[] nums) {
        ...
        for(i=0,j=0;i<n && j<m;j++){
            int start = 0;
            while(j < m && start < groups[i].length && nums[j]==groups[i][start]){
                j++;
                start++;
            }
//            System.out.println(i + " " + j + " start = " + start);
            if(j <= m && start == groups[i].length){
                j--;
                i++;
            }
            else{
            ...
            if(j==m-1){
                if(i!=n || start != groups[n-1].length) return false;
            }
        }
        return true;
  }

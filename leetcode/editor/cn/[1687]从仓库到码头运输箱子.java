...
// 
//
// Related Topics 线段树 队列 数组 动态规划 单调队列 堆（优先队列） 👍 66 👎 0
...
class Solution {
    public int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        int n = boxes.length;
        int[] Weights = new int[n+1];//前i个箱子的重量之和
        int[] Ports   = new int[n+1];//码头数
        int[] Trips   = new int[n+1];//前i个箱子需要的行程次数

        for(int i=1;i<n+1;i++){
            Ports[i]   = boxes[i-1][0];
            Weights[i] = Weights[i-1] + boxes[i-1][1];
            if(i>1){
                if(Ports[i] != Ports[i-1]) Trips[i] = Trips[i-1]+1;
                else Trips[i] = Trips[i-1];
            }
        }
        ...
        dq.addLast(0);
        for(int i=1;i<n+1;i++){
            while(i - dq.peekFirst() > maxBoxes || Weights[i] - Weights[dq.peekFirst()] > maxWeight){
                //当超出容量或者超重
                dq.removeFirst();
            }
            f[i] = g[dq.peekFirst()] + Trips[i] + 2;
            if(i < n){
                g[i] = f[i] - Trips[i+1];
                while(!dq.isEmpty() && g[i] <= g[dq.peekLast()]) dq.removeLast();
                dq.addLast(i);
            }
        }
        return f[n];
